...
namespace Championship_Riven
{
    class Riven
    {
        ...
        {
            if ((Orbwalker.ActiveModesFlags.HasFlag(Orbwalker.ActiveModes.LastHit)||Orbwalker.ActiveModesFlags.HasFlag(Orbwalker.ActiveModes.LaneClear)) && sender is Obj_AI_Turret && sender.Distance(Player.Instance) < 1000 && sender.IsAlly)
            {
                
                if (!(args.Target is AIHeroClient))
                {
                    var Minions = EntityManager.MinionsAndMonsters.GetLaneMinions(EntityManager.UnitTeam.Enemy, Player.Instance.Position, 500);
